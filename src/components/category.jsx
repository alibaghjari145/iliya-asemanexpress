...
function Category(props) {
  ...
  const handleChange = (event) => {
    setCategoryCheck(event.target.checked);
  ...
  return (
    <div >
        ...
        <input type="checkbox" onChange={handleChange} id={props.alpha} />
       ...
       [...Item].map((item, index)=>{
            if(props.alpha== item.id){
                ...
                return(<Items selectedItem={props.selectedItem}  id={index} itemId={item.title} date={item.created} isChecked={CategoryCheck} /> )
            }
        })
    }
       </div>}
